...
settings may be provided via a ``uv.toml`` file in the same folder as the stack definition file.
The format of this configuration file is defined in ``uv``'s
`tool documentation <https://docs.astral.sh/uv/concepts/configuration-files/>`__ and is not
