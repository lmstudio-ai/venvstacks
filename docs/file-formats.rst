...
configuration settings used when invoking ``uv`` are generated by ``venvstacks``, baseline
settings may be provided via a ``venvstacks.uv.toml`` file in the same folder as the stack
definition file. The format of this configuration file is that described for ``uv.toml`` in
|uv's tool documentation|_ and the specific settings available are not further covered here.
