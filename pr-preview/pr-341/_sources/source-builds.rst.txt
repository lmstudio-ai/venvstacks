...
The paths to local wheels are recorded in the layer lock files as relative
paths, so that relative positioning must be maintained between the locking
environment and the layer building environments (for example, if using a git
